...
import type { DistrictRevenue, ReportsOverview } from "@/lib/types";
...
function DistrictRevenueBarList({ items }: { items: DistrictRevenue[] }) {
  if (items.length === 0) {
    return <p className="text-body-sm text-text-muted">Henüz kapanmış anlaşma yok.</p>;
  }
        ...
        <div key={item.district} className="flex items-center gap-3">
          ...
          <div className="h-3 flex-1 overflow-hidden rounded-full bg-surface-container">
            <div
              ...
              style={{ width: `${Math.max((item.revenue / max) * 100, 4)}%` }}
            />
          </div>
          <span className="w-24 shrink-0 text-right text-body-sm font-medium text-on-surface">
      ...
      {!isLoading && data && (data.listing_count > 0 || data.lead_count > 0) && (
        <>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            <StatTile label="Toplam Gelir" value={formatCurrency(data.total_revenue)} />
            <StatTile
              label="Ortalama Komisyon"
              value={data.average_commission != null ? formatCurrency(data.average_commission) : "—"}
            />
            <StatTile label="Kapanan Anlaşma" value={data.closed_deal_count} />
            <StatTile
              label="Dönüşüm Oranı"
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-lg bg-surface-container-lowest p-5 shadow-[0px_10px_30px_rgba(0,0,0,0.04)]">
              <h2 className="mb-4 text-title-md text-primary">Bölgeye Göre Gelir</h2>
              <DistrictRevenueBarList items={data.revenue_by_district} />
            </div>
